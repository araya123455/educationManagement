...
app.post("/assessmentstuinsert", (req, res) => {
  console.log(req.body);
  const assessmentScores = req.body;

  // Prepare values for insertion using a single INSERT statement with multiple rows
  const values = assessmentScores.map((scoreObj) => [
    scoreObj.asses_score,
    scoreObj.stu_id,
    scoreObj.asses_id,
  ]);
  ...
  con.query(sql, [values], function (err, result) {
    ...
    res.status(200).json({
      message: "Successfully added assessment scores",
